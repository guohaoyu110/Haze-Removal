...
def inference(hazy_images):
  ...
  with tf.variable_scope('conv1') as scope:
    kernel = _variable_with_weight_decay('weights',
                                         shape=[5, 5, 1, 8],
                                         wd=WEIGHT_DECAY)
    conv_red = tf.nn.conv2d(hazy_images_red, kernel, [1, 1, 1, 1], padding='SAME')
    conv_green = tf.nn.conv2d(hazy_images_green, kernel, [1, 1, 1, 1], padding='SAME')
    conv_blue = tf.nn.conv2d(hazy_images_blue, kernel, [1, 1, 1, 1], padding='SAME')
    biases = _variable_on_cpu('biases', [8], tf.constant_initializer(0.0))
    pre_activation_red = tf.nn.bias_add(conv_red, biases)
    pre_activation_green = tf.nn.bias_add(conv_green, biases)
    pre_activation_blue = tf.nn.bias_add(conv_blue, biases)
  ...
  with tf.variable_scope('conv2') as scope:
    kernel = _variable_with_weight_decay('weights',
                                         shape=[5, 5, 8, 16],
                                         wd=WEIGHT_DECAY)
    conv_red = tf.nn.conv2d(conv1_red, kernel, [1, 1, 1, 1], padding='SAME')
    conv_green = tf.nn.conv2d(conv1_green, kernel, [1, 1, 1, 1], padding='SAME')
    conv_blue = tf.nn.conv2d(conv1_blue, kernel, [1, 1, 1, 1], padding='SAME')
  ...
  with tf.variable_scope('conv3') as scope:
    kernel = _variable_with_weight_decay('weights',
                                         shape=[5, 5, 16, 32],
                                         wd=WEIGHT_DECAY)
    conv_red = tf.nn.conv2d(conv2_red, kernel, [1, 1, 1, 1], padding='SAME')
    conv_green = tf.nn.conv2d(conv2_green, kernel, [1, 1, 1, 1], padding='SAME')
    conv_blue = tf.nn.conv2d(conv2_blue, kernel, [1, 1, 1, 1], padding='SAME')
  ...
  with tf.variable_scope('linear') as scope:
    kernel = _variable_with_weight_decay('weights',
                                         shape=[1, 1, 32, 1],
                                         wd=WEIGHT_DECAY)
    conv_red = tf.nn.conv2d(conv3_red, kernel, [1, 1, 1, 1], padding='SAME')
    conv_green = tf.nn.conv2d(conv3_green, kernel, [1, 1, 1, 1], padding='SAME')
    conv_blue = tf.nn.conv2d(conv3_blue, kernel, [1, 1, 1, 1], padding='SAME')
    biases = _variable_on_cpu('biases', [1], tf.constant_initializer(0.0))
    linear_red = tf.nn.bias_add(conv_red, biases, name='linear_red')
    linear_green = tf.nn.bias_add(conv_green, biases, name='linear_green')
    linear_blue = tf.nn.bias_add(conv_blue, biases, name='linear_blue')
